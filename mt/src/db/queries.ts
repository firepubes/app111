...
export async function getMessages(db: D1Database, inboxAddress: string): Promise<Message[]> { return db.prepare('SELECT * FROM messages WHERE inbox_address = ? ORDER BY received_at DESC').bind(inboxAddress).all<Message>().then(r => r.results); }

export async function insertMessage(db: D1Database, msg: Omit<Message, 'received_at'>): Promise<void> {
  await db.prepare(`INSERT INTO messages (id, inbox_address, from_address, subject, body, html_body, attachments) VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(msg.id, msg.inbox_address, msg.from_address, msg.subject, msg.body, msg.html_body, msg.attachments).run();
}
...
export async function cleanupExpired(db: D1Database, expiryDays: number): Promise<void> {
  ...
  await db.prepare(`DELETE FROM messages WHERE inbox_address IN (SELECT address FROM inboxes WHERE datetime(created_at) < datetime('now', '-' || ? || ' days'))`).bind(expiryDays).run();
  await db.prepare(`DELETE FROM session_inboxes WHERE inbox_address IN (SELECT address FROM inboxes WHERE datetime(created_at) < datetime('now', '-' || ? || ' days'))`).bind(expiryDays).run();
  await db.prepare(`DELETE FROM inboxes WHERE datetime(created_at) < datetime('now', '-' || ? || ' days')`).bind(expiryDays).run();
